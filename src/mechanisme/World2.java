...
public class World2 {
	...
	public void run(){
		...
		for(int x = herb4m.size()-1; x >= 0; x--){
			for(int y = herb4f.size()-1; y >= 0; y--){
				...
				if(d < h4f.getR()){
					...
					if(parent.random(1) < 0.1 && h4m.lastChildTime() > 10000 && h4f.lastChildTime() > 10000){
		...
		/////drink water////////////////
		for(int a = carn2f.size()-1; a >= 0; a--){
			for(int b = lac.size()-1; b >= 0; b--){
				Carnivor2F c2f = carn2f.get(a);
				Lac l = lac.get(b);
				PVector lpos = l.position;
				PVector c2fpos = c2f.position;
				float dist = PVector.dist(lpos, c2fpos);
				if(c2f.getR() > dist){
					c2f.drinkWater();
				}
			}
		...
		if(carn2f.size() > 0){
			...
			for(int a = carn2f.size()-1; a >= 0; a--){
				...
				if(c2f.soif() == true){
					...
					for(int b = lac.size()-1; b >= 0; b--){
						Lac l = lac.get(b);
						dist = ((c2f.position.x - l.position.x) * (c2f.position.x - l.position.x)) + ((c2f.position.y - l.position.y) * (c2f.position.y - l.position.y));
						if( dist < currentSmallest){
							currentSmallest = dist;
							target.x = l.position.x;
							target.y = l.position.y;
		...
		for(int a = carn2f.size()-1; a >= 0; a--){
			...
			for(int b = meat2.size()-1; b >= 0; b--){
				Meat2 m2 = meat2.get(b);
				PVector cpos = c2f.position;
				PVector mpos = m2.position;
				
				float d = PVector.dist(cpos, mpos);
				if(c2f.fullbelly() == false){
					if(d < c2f.getR()){
						c2f.eat2();
						meat2.remove(b);
					}
		...
		if(carn2f.size() > 0){
			...
			for(int a = carn2f.size()-1; a >=0; a--){
				Carnivor2F c2f = carn2f.get(a);
				if(meat2.size() > 0 && c2f.fullbelly() == false){
					PVector target = new PVector(0,0);
					currentSmallest = (worldH * worldH) + (worldW * worldW);
					for(int b = meat2.size()-1; b >=0; b--){
						Meat2 m = meat2.get(b);
						dist = ((c2f.position.x - m.position.x) * (c2f.position.x - m.position.x) + (c2f.position.y - m.position.y) * (c2f.position.y - m.position.y));
						if(dist < currentSmallest){
							currentSmallest = dist;
							target.x = m.position.x;
							target.y = m.position.y;
		...
		for(int a = carn2f.size()-1; a >=0; a--){
			...
			for(int b = herb4f.size()-1; b >=0; b--){
				...
				PVector c2fpos = c2f.position;
				PVector h4fpos = h4f.position;
				float d = PVector.dist(c2fpos, h4fpos);
				if(c2f.faim() == true){
					if(d < c2f.getR()){
						h4f.health -= 100;
					}
			...
			for(int c = herb4m.size()-1; c >=0; c--){
				...
				PVector c2fpos = c2f.position;
				PVector h4mpos = h4m.position;
				float d = PVector.dist(c2fpos, h4mpos);
				if(c2f.faim()==true){
					if(d < c2f.getR()){
						h4m.health -= 100;
					}
		...
		if(carn2f.size() > 0){
			...
			for(int a = carn2f.size()-1; a >=0; a--){
				...
				if(c2f.faim() == true){
					...
					for(int b = herb4f.size()-1; b >=0; b--){
						Herbivor4F h4f = herb4f.get(b);
						dist = ((c2f.position.x - h4f.position.x) * (c2f.position.x - h4f.position.x) + (c2f.position.y - h4f.position.y) * (c2f.position.y - h4f.position.y));
						if(dist < currentSmallest){
							currentSmallest = dist;
							target.x = h4f.position.x;
							target.y = h4f.position.y;
					...
					for(int c = herb4m.size()-1; c >=0; c--){
						Herbivor4M h4m = herb4m.get(c);
						dist2 = ((c2f.position.x - h4m.position.x) * (c2f.position.x - h4m.position.x) + (c2f.position.y - h4m.position.y) * (c2f.position.y - h4m.position.y));
						if(dist2 < currentSmallest){
							currentSmallest = dist2;
							target.x = h4m.position.x;
							target.y = h4m.position.y;
		...
		//////reproduction carnivor2/////
		/////////////////////////////////
		
		for(int a = carn2m.size()-1; a >=0; a--){
			for(int b = carn2f.size()-1; b >=0; b--){
				Carnivor2M c2m = carn2m.get(a);
				Carnivor2F c2f = carn2f.get(b);
				PVector c2mpos = c2m.position;
				PVector c2fpos = c2f.position;
				float d = PVector.dist(c2mpos, c2fpos);
				if( d < c2f.getR()){
					DNA3 momgenes = c2f.getDNA3();
					DNA3 dadgenes = c2m.getDNA3();
					DNA3 childDna3 = momgenes.crossover(dadgenes, parent);
					if(parent.random(1) < 0.5 && c2m.lastChildTime() > 10000 && c2f.lastChildTime() > 10000){
						if(parent.random(10) > 5 && carn2f.size() < 10){
							carn2f.add(new Carnivor2F(c2fpos, childDna3, parent));
							c2f.setLastChildTime(parent.millis());
						}
						if(parent.random(5) < 5 && carn2m.size() < 10){
							carn2m.add(new Carnivor2M(c2fpos, childDna3, parent));
							c2m.setLastChildTime(parent.millis());
						}
					}
				}
			}
		...
		parent.text("carnivor female " + carn2f.size(), 10, 50);
		//parent.text("mouse x" + parent.mouseX, 10, 40);
		parent.text("Meat " + meat2.size(), 10, 60);
